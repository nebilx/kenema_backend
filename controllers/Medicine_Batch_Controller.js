...
const getAllMedicine_Batch = asyncHandler(async (req, res) => {
  ...
  if (!medicine_Batch) return res.status(204).json({ message: "No Medicine_Batch found" });
...
const insertMedicine_Batch = asyncHandler(async (req, res) => {
  ...
  if (Medicine_BatchExists) {
    res.status(409);
    throw new Error("Medicine_Batch already exists"); // 409 conflict
  }

  try {
    const medicine_Batch = await Medicine_Batch.create({ drug_expire, drug_quantity });

    res.status(201).json({ success: `New Medicine_Batch created!` + medicine_Batch });
  } catch (err) {
    res.status(500).send(err);
    console.error(err);
  }
...
const updateMedicine_Batch = asyncHandler(async (req, res) => {
  ...
  if (!id) return res.status(400).json({ message: "Medicine_Batch ID required" });
  ...
  if (!Medicine_BatchExists) {
    return res.status(204).json({ message: `Medicine_Batch ID ${id} not found` });
  }

  try {
    const medicine_Batch = await Medicine_Batch.findByIdAndUpdate(
      { _id: id },{drug_expire, drug_quantity}
    );

    medicine_Batch.save();

    res.status(201).json({ success: `Medicine_Batch Updated` + medicine_Batch });
  } catch (err) {
    res.status(500).send(err);
    console.log(err);
  }
...
const deleteMedicine_Batch = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ success: `Medicine_Batch Deleted!` + medicine_Batch });
  } catch (err) {
    res.status(500).send(err);
    console.log(err);
  }
